 ...
 */
export function getCustomDisplayIndex(item, displays) {
  // const hasMainExternal = displays.some(_ => _["monitor-name"].includes("Studio"))
  const hasMainStudio = displays.some(_ => _["monitor-name"].includes("Studio"))

  const hasMainBuiltin = !hasMainStudio && displays.some(_ => _["monitor-name"].includes("Built-in"))


  // Map through displays and render spaces for the current display
  // basically; the built-in display will probably be 1.
  // the external displays will be >2
  const nativeId = item["monitor-appkit-nsscreen-screens-id"]
let displayId = hasMainStudio 
      ? item["monitor-name"]?.includes("Studio") 
        ? 3 
        : 1
      : hasMainBuiltin
        ? item["monitor-name"]?.includes("Built-in")
          ? 3
          : 1
        : nativeId
  console.log("_getCustom", { item, displays, displayId})

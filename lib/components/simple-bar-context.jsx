...
export default function SimpleBarContextProvider({
  ...
  const currentDisplay =
    currentDisplays?.find((d) => {
      const id = Aerospace.getCustomDisplayIndex(d, displays) ?? d["monitor-id"];
      console.log({ id, did: d.id });
      return id === adjustedDisplayId;
    }) || {};

  const displayIndex =
    (currentDisplay.index ?? Aerospace.getCustomDisplayIndex(currentDisplay, displays)) ||
    1;
